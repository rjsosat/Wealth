...
export const useFinanceCalculations = () => {
    ...
    const currentMonthExpenses = useMemo(() => {
        ...
        return transactions
            .filter((t) => t.type === 'Expense' && t.date.substring(0, 7) === nowStr)
            .reduce((total, t) => {
    ...
    const currentMonthIncome = useMemo(() => {
        ...
        return transactions
            .filter((t) => t.type === 'Income' && t.date.substring(0, 7) === nowStr)
            .reduce((total, t) => {
                const amountInPreferred = convertCurrency(t.amount, t.currency, preferredCurrency);
                return total + amountInPreferred;
            }, 0);
    ...
    const checkBudgetProgress = (categoryId: string) => {
        ...
        return {
            ...
            currency: 'PEN',
            percentage: (spentThisMonth / limitAmount) * 100,
        };
    };
    ...
    const last7DaysSpending = useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const spentOnDate = transactions
                .filter((t) => {
                    // Extract YYYY-MM-DD exactly to prevent local timezone shifting
                    const localDateStr = t.date.split('T')[0];
